...
@implementation MGDayPadController
...
#define DayDigitLengthLimit 2
    lengthLimit = DayDigitLengthLimit;
}
...
- (IBAction)tappedButtonSave:(id)sender {
    ...
    if ([scanner scanInteger:&scoreInteger]) {
        _updatedScore = scoreInteger;
    } else {
        _updatedScore = 0;
    }
#ifdef DEBUG
    NSLog(@"%s: _updatedScore:%d", __FUNCTION__, _updatedScore);
#endif
    SEL sel = @selector(updateResetDay:);
    if ([self.delegate respondsToSelector:sel]) {
        [self.delegate updateResetDay:_updatedScore];
    }
    [self dismissModalViewControllerAnimated:YES];
}
@end
